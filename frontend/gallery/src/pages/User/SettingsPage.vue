<template>
  <div class="setting">
    <div class="setting_grid" ondragstart="return false">
      ...
      <div class="setting_con setting_left_size" v-show="checkPassword_state">
        <div class="setting_menu">
          ...
          <div data-value="2">
            <span class="light"></span><span>작가등록</span>
          </div>
          <div data-value="3">
            <span class="light"></span><span>회원탈퇴</span>
          </div>
        </div>
      </div>
        ...
        <ul class="setting_content">
            ...
            <div class="modUserInfo">
              <div class="modUserContainer1">
                <div class="file_input">
                  ...
                  /><br />
                  <button class="uploadImg" @click.prevent="onClickUploadImage">
                    프로필 사진 업로드</button
                  ><br />
                </div>
              </div>
              <div class="modUserContainer2">
                ...
                <div class="UserContainer2 mod_nickname">
                  ...
                  <input
                    ...
                    class="nickname_self"
                    readonly="true"
                    v-model="self_nickName"
                  />
                  <input
                    type="text"
                    id="nickname"
                    readonly="true"
                    v-model="this.state.user_nickName"
                  />
                  <button class="random_btn" @click.prevent="makeUserNickName">
                    <i class="fas fa-random"></i>
                  </button>
                  <button
                    class="selfmod_btn"
                    @click.prevent="modifyUserNickName"
                  >
                    직접수정
                  </button>
                ...
                <div class="UserContainer2">
                  ...
                  <p id="mod_explain">
                    * 19금관 이용을 원하신다면 반드시 입력해주세요.
                  </p>
                </div>
              </div>

              <div class="save_btn_container">
                <div class="mod_info_button" @click="modifyUser">
                  <span>수정완료</span>
                </div>
              </div>
            </div>
            ...
            <div class="registerArtist">
              ...
              <div class="register_artist_container">
                ...
                <div class="register_artist" id="rank_info">
                  ...
                  <button type="button" id="rank_modal_btn">
                    <i class="fas fa-info-circle"></i>
                  </button>
                  <div class="rank_modal_wrap">
                    <div class="rank_modal_close">
                      <a href="#"><i class="fas fa-times"></i></a>
                    </div>

                    <div class="rank_modal_content">
                      <br />
                      <b>아마추어</b> : 작품 업로드 개수가 10개 이하인 아마추어
                      작가 <br /><br />
                      <b>고수</b> : 작품 업로드 개수가 10개 이상인 고수 작가
                      <br /><br />
                      <b>프로</b> : 메인 갤러리 추천에 오른 프로 작가<br />
                    </div>
                  </div>
                  <p class="registerinfo">
                    * 등급은 작가등록 후 gallery의 등급기준에 따라 자동으로
                    부여됩니다.
                  </p>
                </div>
                <div class="register_save_btn_container">
                  <div
                    class="mod_info_button"
                    v-if="user__type === '1'"
                    @click="register_artist"
                  >
                    <span>작가등록</span>
                  </div>
                  <div v-else>
                    <div class="mod_a_info_button" @click="modify_artist">
                      <span>작가 수정</span>
                    </div>
                    <div class="mod_info_button" @click="withdraw_artist">
                      <span>작가 해지</span>
                    </div>
                  </div>
                </div>
          ...
          <li class="leave_user_li">
            <div class="unscribe_container">
              <div class="unscribe_container2">
                ...
                <div class="unscribe_reason">
                  ...
                  <div class="unscribe_btn_container">
                    <div class="mod_info_button" @click="askWhy">
                      <span>탈퇴하기</span>
                    </div>
                  </div>
              ...
              <div class="delete__check1">정말 나갈꺼니?</div>
              <div class="delete__check2">정말 나갈꺼니?</div>
              <div class="delete__check3">정말 나갈꺼니?</div>
            </div>
          </li>
        </ul>
      </div>
    ...
    -->
      ...
      <div slot="header" v-else-if="artist_resister === 'leaveArtist'">
        <h3>작가 해지</h3>
        <i class="fas fa-times closeModalBtn" @click="showModal = false"></i>
      </div>
      ...
      <div slot="body" v-if="artist_resister === 'changeArtist'">
        ...
        <p>
          ...
          다하고 있습니다. 문의가 있다면 언제나 저희에게 연락을 주십시오. 다시
          한번 환영합니다!!
        </p>
      </div>
      ...
      <div slot="body" v-else-if="artist_resister === 'leaveArtist'">
        <h3>{{ artist_nickName }}작가님!</h3>
        <p>
          저희 '갤러리 Ω,'에서의 작가활동은 어떠셨나요? 다신 한번 만나뵙기를
          기원하겠습니다. 작가님의 앞으로의 작가활동을 저희 저희 '갤러리 Ω,'이
          응원하겠습니다!
        </p>
      </div>
      ...
      <div slot="footer">
        <button
          ...
          @click="[(showModal = false), resetpage()]"
        >
          닫기
        </button>
      </div>
...
export default {
  ...
  methods: {
    ...
    withdraw_artist() {
      http
        .post(
          `/artist/leaveArtist?artist_id=` + localStorage.getItem("user_id")
        )
        .then((response) => {
          this.artist_resister = response.data.name;
          // console.log(response.data);
