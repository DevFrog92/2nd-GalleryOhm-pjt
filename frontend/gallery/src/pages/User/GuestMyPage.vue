    ...
    <div class="guest_profile_wrapper">
      ...
      >
        <div class="profile_border">
          ...
          <div class="profile_icons">
            <i
              class="profile_icon_guest icon-chat"
              aria-hidden="true"
              @click="DMsideopen"
            ></i>
          </div>
      ...
      <div class="profile_about">
        <h2 class="profile_user_name">
          ...
          }}<span class="Mypage__profile_batch">관람객</span>
          <span v-show="!who_state" class="Guest__bell" @click="DMsideopen"
            ><img src="../../assets/images/bell.png" alt="" /><span
              class="notification__num"
              >{{ Unread_count }}</span
            ></span
          >
        </h2>
          ...
          <div v-if="modifyabout" class="modal_modify_about">
            ...
            <div class="modify_btn" @click="registerUserAbout">
              <div class="segment__guest">
                <button class="unit__guest unit__btn__guest" type="button">
                  <img
        ...
        <span
          v-if="!modifyabout"
          class="icon-pencil-alt-span"
          @click="[(modifyabout = !modifyabout), (modify_state = false)]"
        >
        ...
        <div class="profile_menu_footer">
          <div class="profile_menu">
            ...
            <div class="profile_item">
              <span class="profile_menu_item" data-value="3">즐겨찾기</span>
            </div>
            <div v-if="!who_state" class="profile_item">
              <span class="profile_menu_item" @click="moveSettings"
                >회원정보 수정</span
              >
            </div>
          </div>
        ...
        <div class="showDmContent_Guest">
          ...
          <div class="message__content__cover__guest">
            <div class="sender_content">
              저에게 문의 혹은 궁금하신 점이 있다면 서신을 날려주세요.
            </div>
          </div>
          <div class="message__reply__guest">
            <div class="message__reply__titie">
              ...
              <input
                type="text"
                id="response_message_title__guest"
                v-model="dm_title"
              />
            </div>
        ...
        <div class="contact_to_me">
          ...
          <ul class="Guest__DMList">
            <div
              :class="
                !dm.message_isCheck
                  ? 'Guest__DM__item Guest__DM__item__read '
                  : 'Guest__DM__item'
              "
              v-for="(dm, index) of dm_list"
              :key="index"
              :data-name="dm.message_id"
            ...
            >
              ...
              <div class="Guest__DM__item__content">
                <span class="Guest__DM__item__title">{{
                  dm.message_senderId
                }}</span>
                <span class="Guest__DM__item__message"
                  >{{ dm.message_title }}<br />{{ dm.message_sendDate }}</span
                >
              </div>
              <div
                class="Guest__DM__item__option delete Guest__dm-option"
                :data-value="dm.message_id"
        ...
        <div class="Guest__showDmContent">
          ...
          <div class="Guest__message__reply">
            <div class="Guest__message__reply__titie">
              <label for="response_message_title" class="Guest__response_title"
                >제목 :
              </label>
              <input
                type="text"
                id="Guest__response_message_title"
                v-model="dm_title"
              />
            </div>
    ...
    <div class="forth__section__guest">
      ...
      <div class="outer-wrapper-guest" v-if="!scrap_state">
        <div class="scroll-wrapper">
          <div
            class="scroll-slide"
            v-for="(item, index) of scrap_list"
            :key="index"
          >
            <img
              class="scrap__image"
              :data-value="item.work_id"
              :src="'data:/image/jpeg;base64,' + item.work_piece"
              alt=""
            />
          </div>
        </div>
...
export default {
  ...
  created() {
    if (
      this.props_id &&
      localStorage.getItem("user_id") !== localStorage.getItem("props_id")
    ) {
      this.who_state = true;
